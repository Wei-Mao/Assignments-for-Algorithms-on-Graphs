...
class Clusetering
{
  private:
    int size;
    int num_of_clusters; //  # of the clusters
    vector<Edge> edges;
    vector<int> x;
    vector<int> y;
  ...
  public:
    ...
    double Kruskal_Cluster()
    {
      // Reference: https://courses.cit.cornell.edu/info295_2006fa/capp.pdf
	...
	if(dis_sets_vertices.find(u) != dis_sets_vertices.find(v))
	{
	  ...
	  if(dis_sets_vertices.num_of_sets == (num_of_clusters))
	  {
	    // The given set of points is already grouped into num_of_clusters clusters in the last iteration.
	    spacing = edges[i].length;
	    break;
	  }
...
int main() {
  Clusetering Cluster;
  Cluster.Data_Read();
  std::cout << std::fixed << std::setprecision(10) << Cluster.Kruskal_Cluster() << std::endl;
